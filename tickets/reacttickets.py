...
class ReactTickets(commands.Cog):
    ...
    async def ticketset_nsfw(self, ctx: commands.Context, inputNSFW = ""):
        ...
        inputstr = inputNSFW.lower()


        if inputstr == "true" or inputstr == "on" or inputstr == "y" or inputstr == "ye" or inputstr == "yes":
            newNSFW = True

        if inputstr == "false" or inputstr == "off" or inputstr == "n" or inputstr == "no":
            newNSFW = False

        settings = newNSFW
        await self.config.guild(ctx.guild).markasnsfw.set(newNSFW)
    ...
    async def _open_ticket(
        ...
        channelname = "open-{}".format(user.id)
        category = guild.get_channel(settings["open_category"])
        nsfwYN = settings["markasnsfw"]
