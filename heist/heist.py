    ...
    async def _bailout_heist(self, ctx, user: discord.Member=None):
        ...
        if player.id == author.id:
            msg = ("Do you want to make a {0} amount? It will cost {1} credits. If you are "
                   "caught again, your next {2} and {0} amount will double. "
        ...
        if "yes" in response.content.lower() or "y" in response.content.lower():
    ...
    async def heist_game(self, ctx, guild, t_heist, t_crew, t_vault):
        ...
        if len(curcrew) != 0:
            ...
            msg = ("The credits collected from the {} was split among the winners:\n```"
                   "C\n{}```".format(t_vault, t))
            targets[target]["Vault"] = (int)(targets[target]["Vault Max"] * 0.25);
        else:
            msg = "No one made it out safe."
            targets[target]["Vault"] = targets[target]["Vault"] + (int)(crew * config["Cost"] * 0.5);
            if (targets[target]["Vault"] > targets[target]["Vault Max"]):
                targets[target]["Vault"] = targets[target]["Vault Max"];
